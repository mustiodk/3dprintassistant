...
const Engine = (() => {
  ...
  // Internal support lookup — id/name only. Chip descs are generated by
  // getFilters() from _SUPPORT_GEOMETRY below so numbers stay in one place.
  const _SUPPORT_TYPES = [
    { id: 'none',           name: 'None' },
    { id: 'easy',           name: 'Easy removal' },
    { id: 'balanced',       name: 'Balanced' },
    { id: 'best_underside', name: 'Best underside' },
  ];
  ...
  // Support-type geometry — single source of truth read by both the UI chip
  // desc (getFilters) and the profile emission (resolveProfile). Changing a
  // value here updates both simultaneously. IMPL-040.
  const _SUPPORT_GEOMETRY = {
    easy:           { type: 'Tree',   z_gap: '0.30' },
    balanced:       { type: 'Tree',   z_gap: '0.20' },
  ...
  function getFilters(state) {
    ...
    // Support geometry — read from module-scoped _SUPPORT_GEOMETRY so both the
    // UI chip desc (below) and resolveProfile() emission read the same source.
    const SUPPORT_Z_GAP = { easy: _SUPPORT_GEOMETRY.easy.z_gap, balanced: _SUPPORT_GEOMETRY.balanced.z_gap, best_underside: _SUPPORT_GEOMETRY.best_underside.z_gap };
    const SUPPORT_TYPE  = { easy: _SUPPORT_GEOMETRY.easy.type,  balanced: _SUPPORT_GEOMETRY.balanced.type,  best_underside: _SUPPORT_GEOMETRY.best_underside.type };
    ...
    return [
      ...
      { key: 'support',     label: t('filterSupport'),   multi: false, required: false, items: [
        ...
        { id: 'easy',           name: t('supEasy'),          desc: `${SUPPORT_TYPE.easy} · Z ${SUPPORT_Z_GAP.easy} mm — supports snap off easily, rougher underside.` },
        { id: 'balanced',       name: t('supBalanced'),      desc: `${SUPPORT_TYPE.balanced} · Z ${SUPPORT_Z_GAP.balanced} mm — reasonable removal with decent underside.` },
        { id: 'best_underside', name: t('supBestUnderside'), desc: `${SUPPORT_TYPE.best_underside} · Z ${SUPPORT_Z_GAP.best_underside} mm — smooth underside, supports harder to remove.` },
      ]},
  ...
  function resolveProfile(state) {
    ...
    if (support && support.id !== 'none') {
      ...
      // Z-gap from single source of truth _SUPPORT_GEOMETRY — same table the UI
      // chip desc reads, so "Easy removal · Z 0.30 mm" in the chip and the
      // emitted support_z_distance can never drift. IMPL-040.
      const zGap = _SUPPORT_GEOMETRY[support.id]?.z_gap || '0.10';
