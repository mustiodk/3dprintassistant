...
const Engine = (() => {
  ...
  function _prusaParamValue(param) {
    ...
    if (param._slicer_value != null) return String(param._slicer_value);
    return _numericValue(param.value ?? param);
  }
  ...
  function exportPrusaINI(state) {
    ...
    const profile = resolveProfile(exportState);
    const adv = getAdvancedFilamentSettings(exportState);
    const bs = material.base_settings || {};
    ...
    Object.entries(PRUSA_PRINT_MAP).forEach(([engineKey, prusaKey]) => {
      const value = _prusaParamValue(profile[engineKey]);
    ...
    lines.push(
      ...
      `first_layer_bed_temperature = ${parseInt(adv.initial_layer_bed_temp, 10)}`,
    );

    if (bs.flow_ratio != null) lines.push(`extrusion_multiplier = ${bs.flow_ratio}`);
    const retraction = _prusaParamValue(profile.retraction_distance)
      || (bs.retraction_distance != null ? String(bs.retraction_distance) : null);
    if (retraction != null) lines.push(`filament_retract_length = ${retraction}`);
    if (bs.retraction_speed != null) lines.push(`filament_retract_speed = ${bs.retraction_speed}`);
    const mvs = bs.max_mvs?.[String(nozzle.size)];
    if (mvs) lines.push(`filament_max_volumetric_speed = ${parseFloat(mvs)}`);
    if (adv.fan_min_speed != null) lines.push(`min_fan_speed = ${parseInt(adv.fan_min_speed.value, 10) || 0}`);
    if (adv.fan_max_speed != null) lines.push(`max_fan_speed = ${parseInt(adv.fan_max_speed.value, 10) || 100}`);
