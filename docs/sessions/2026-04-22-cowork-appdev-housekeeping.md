...
- **iOS working-tree drift cleanup.** Beyond this session's scope but worth a dedicated later pass: 9 flat-layout screenshot deletions + 12 untracked items under iOS root/docs/. Want me to enumerate + triage in the next session, or let it ride?
- **`asc keys/` folder location.** The new `.p8` lives inside the iOS repo at `asc keys/AuthKey_SF8C66YLJY.p8`. It's gitignored (caught by `AuthKey_*`), so safe, but the Apple / Fastlane convention is to keep keys at `~/.appstoreconnect/private_keys/AuthKey_<KEYID>.p8` so `spaceship`/`fastlane` finds them without config. Optional cleanup for a later session.
