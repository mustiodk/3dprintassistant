...
- **Follow-on (same session): memory hygiene via Work Protocol (Code/Light lane).** Slimmed `memory/project_3dprintassistant.md` from the stale 2026-04-03 snapshot to a **portable decode + pointers** (option 2) — its content had been superseded by ROADMAP (live) + `docs/3dpa-context.md` (evergreen) + the CLAUDE.md hot-cache, with no feedback loop updating it (the drift cause). Updated the `MEMORY.md` index hook to match. Fixed a stale pointer in CLAUDE.md + AGENTS.md (`memory/projects/` → `memory/project_*.md`; the subdir never existed). Commits: claude-sync `79a310c7` (memory slim + index); claude-projects `de9b23f` (pointer fix, byte-identical, on `main`, pushed). **Then ran the full `consolidate-memory` pass** across all 59 memory files (3 parallel read-only skim-agents): fixed 9 files' migration-stale `Documents/Claude` paths (→ `dev/Claude`; relative cross-repo links made absolute `~/dev/Claude/...`) + replaced MEMORY.md's stale 2026-04-03 project-map with a pointer to the maintained CLAUDE.md Active-Projects hot-cache. **Zero merges/retirements** — the codex-review (×6) + bridge (×3) clusters were verified genuinely distinct, and both agent-proposed retirements (`feedback_next_session_on_ask`, `feedback_vault_cold_start`) were **rejected after reading the actual files** (the first is the base rule that already resolves its own exception; the second is really Trigger D / vault-catch-up, not cold-start — the filename misled the agent). claude-sync commit `e8761daa`, pushed. **Correction (same session): I made — then fixed — a regression.** I'd flagged `memory/glossary.md` as "missing" and "fixed" the CLAUDE.md pointer `→ Details: memory/projects/` to `memory/project_*.md`. Both were WRONG: I had conflated two distinct memory systems. There are TWO: (1) the auto-memory store at `~/.claude/projects/-Users-mragile-io-dev-Claude-Projects/memory/` (MEMORY.md + flat `feedback_/project_/reference_` files — what the consolidation pass operated on); (2) a project hot-cache at `~/dev/Claude/Projects/memory/` containing `glossary.md` + `projects/{3dprintassistant-ios,3dprintassistant-web,bambuinventory}.md`, referenced by CLAUDE.md's embedded Memory hot-cache (relative paths, both correct). So `glossary.md` exists where CLAUDE.md points, and `memory/projects/` was the correct pointer. **Reverted** the bad pointer edit (CLAUDE.md + AGENTS.md restored to `memory/projects/`, byte-identical) and renamed the genuinely-misnamed auto-memory file `feedback_vault_cold_start.md` → `feedback_trigger_d_vault_catchup.md` (content is Trigger D, not cold-start; only MEMORY.md index referenced it). **New open observation (flagged, not acted on):** the two memory systems partially duplicate (e.g. auto-memory `project_3dprintassistant.md` vs project hot-cache `projects/3dprintassistant-web.md` + `-ios.md`) — worth an owner decision on whether to dedupe/unify, but that's a structural call beyond this pass. Lesson: verify which of the two memory dirs a path resolves to before asserting missing/broken.
