...
Plan documents 7 web tasks + 1 iOS sync task. 5 of 7 shipped through S4 (Tasks 1, 2, 3, 4, 5). Remaining: Task 6 (chamber safe-cap, warning-only — this session's target), Task 7 (nozzle min-diameter cleanup + nozzle authority cleanup — bundles HIGH-12 + Claude HIGH-06, last Phase 1 web commit). **After Phase 1 completes (Task 7 lands), arc transitions to Phase 2 iOS sync** (gated — owner approves push + dispatches TestFlight). Each web task is one commit per platform.
