...
**Adversarial review — `bridge --mode codex-only`, 5 rounds, converging 5→3→2→1→0:** R1 NO-GO (P1 unlink-by-path TOCTOU; P2 under-validated structured freeze; P2 `\S+` legacy overreach; P2 raw transport-error logging; P3 predictable temp) → `b49f935`/`420dc30`/`ef55752`/`1e3f314`/`0d68c2a`. R2 NO-GO (P1 same-bytes compare still racy; P2 hybrid freeze fell through to the legacy parser; P3 temp still predictable) → `52ca6ee` (mutation lock)/`cc3081a`/`ae418df`. R3 NO-GO (P1 the lock was not a true mutex — creator timeout bypass + stale takeover vs a paused deleter; P2 a wedged lock could strand recovery in repost loops) → `bad2d62` **removed the lock entirely** in favour of atomic claim-verify-unlink plus preflight fail-closed on stranded claims. R4 NO-GO (P1 a crash between the freeze temp write and rename left `.tmp` invisible to preflight — fail-open) → `535b6fd` (preflight fails `78` on ANY `.intake-autonomy-freeze.*` sibling). R5 **GO**, no P0/P1/P2; R4 had already cleared the deletion protocol, the fail-closed paths, and all earlier fixes against regression. Every accepted finding = one commit with a RED-first test (two degenerate-RED cases noted in their commit bodies per the CLAUDE.md breadcrumb rule).

**Verification:** 11 suites green — notifier 34/34, wrapper 1–13, installer 1–10, preflight (incl. new claim + tmp cases), bootstrap, kv-hygiene, diff-guards, republish-overlay, verify-live-overlay, parked-store, owner-decision; `git diff --check origin/main...HEAD` clean.

**Release:** web branch merged `--no-ff` to `main` as `53e032b` and pushed; ai-om contract v2.7 merged in an isolated parent worktree as `3dee67c` and pushed (unrelated `daniasportsrejser*` parent dirt untouched).
