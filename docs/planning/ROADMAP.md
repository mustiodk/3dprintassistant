...
**Last updated:** 2026-05-13 — v1.0.3 is live on the App Store; remote overlay `content_version=2026051202` ships i7 under Creality / i Series for current iOS clients. v1.0.4 S4 (impl) complete: web `1695cba` ships Task 5 (practical multicolor 5-tier resolver — mutual HIGH ams_lite_compatible + Claude HIGH-03 system-type + Codex MEDIUM-01 empty-MCS) on top of S3's Task 4. 5 of 7 Phase-1 web tasks shipped; 6 cumulative v1.0.4 walkthrough OK lines; 55/55 curated + 47196/0 broad audit. iOS untouched (Phase 2 gated). **Phase shape locked at 4 phases:** Phase 1 (web Tasks 1-7) → Phase 1.5 (Codex audit, owner-gated dispatch) → Phase 2.1 (iOS engine/data sync + XCTest) → Phase 2.2 (UI walkthrough + MARKETING_VERSION bump + ship). **Next step:** S5 cold-starts and executes Task 6 (chamber safe-cap guard, warning-only).
...
- **v1.0.4 config-impact implementation.** Setup complete (S1, 2026-05-13): [`merged.md`](../reviews/2026-05-11-config-impact-qa/merged.md) is the owner-accepted queue (locked SHA `5bcd68b`); [`2026-05-13-v1.0.4-config-impact.md`](../superpowers/plans/2026-05-13-v1.0.4-config-impact.md) is the per-task implementation plan (TDD-first per finding, 7 web tasks + 1 Codex audit gate + 2 iOS phases). Owner-locked planning defaults are reproduced verbatim in `merged.md`. S2 (2026-05-13) shipped Tasks 1, 2, 3 (web `133be38` → `b238a82`): strength `speed_multiplier`, env data layer + cold-warning copy + env clamp attribution, physical printer × nozzle guard. S3 (2026-05-13) shipped Task 4 (web `bc070af` + `bf05586`): physical printer × plate guard + material plate range bundle (HIGH-02 + HIGH-03 material half) — fixup mirrored the full bed-temp recipe (initial-layer offset + PETG +5 + env adj + clamp) after the first-pass steady-state-only recipe missed 3 silent-regression PETG-on-plate combos. S4 (2026-05-13) shipped Task 5 (web `dc49c52` + `1695cba`): practical multicolor 5-tier resolver. New `_mcsTier(printer)` returns `none`/`ams_lite`/`ams_like`/`cfs`/`generic_non_ams`; empty-MCS printers suppress prime_tower + flush_into_infill + AMS checklist; A1/A1 Mini gate by `material.ams_lite_compatible`; CFS/CFS Lite/MMU3/ACE/IDEX/Toolchanger/Filament Hub get tier-appropriate guidance copy; retired Creality-only manufacturer hardcode + `k2_plus_cfs`. Fixup closed curated audit drift + sysLabel display-name polish. 5 of 7 Phase-1 web tasks shipped; walkthrough now has 6 cumulative v1.0.4 OK lines (9 sub-assertions in MCS block); profile-matrix-audit clean across 55/55 curated + 47196 broad configs. **Next step:** S5 cold-starts and executes Task 6 (chamber safe-cap guard, warning-only — HIGH-05; single engine site, no profile field per owner default 4). If context permits, S5 may also pick up Task 7 (nozzle min-diameter cleanup + nozzle authority cleanup — last Phase 1 web commit). Multi-session autonomous arc; per-finding checkpoint rule + Trigger A close between sessions. iOS push gate enforced — local commits only until 5-point ship-ready check passes. **After Phase 1 completes (Task 7 lands):** Phase 1.5 (Codex audit, owner-gated dispatch) prepares a single review packet at `codex/v1.0.4-audit/` and stops; owner dispatches Codex manually; S7 cold-starts on the response and remediates web-only (stop conditions: ≥1 CRITICAL or ≥5 HIGH pauses for owner; 1-4 HIGH remediates autonomously; MEDIUM/LOW owner-triaged). Then Phase 2.1 (Task 8 — iOS engine/data sync + XCTest + iOS local commit) → Phase 2.2 (Task 9 — UI walkthrough + MARKETING_VERSION bump + iOS local commit + 5-point ship-ready handoff). Owner manually dispatches TestFlight after Phase 2.2.
