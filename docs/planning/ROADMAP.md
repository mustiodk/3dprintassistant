...
**Last updated:** 2026-05-13 — v1.0.3 is live on the App Store; remote overlay `content_version=2026051202` ships i7 under Creality / i Series for current iOS clients. **v1.0.4 Phase 1.5 owner-triage complete; HIGH-01 web-only remediation shipped (S7-1).** Codex audit response captured at `codex/v1.0.4-audit/codex-2026-05-13-v1.0.4-audit-response.md` (commit `fe2964e`); triage returned 0 CRITICAL / 2 HIGH / 2 MEDIUM / 1 LOW / 1 OBSERVATION. Owner-approved scope: HIGH-01 + HIGH-02 + MEDIUM-01 + LOW-01 in v1.0.4; defer MEDIUM-02 (matrix-audit sweep extension) + OBSERVATION-01 (plate helper API asymmetry) to v1.0.5. HIGH-02 modeling locked to Option B (data + engine + harness + material-aware cold-env warning copy rewrite). S7-1 shipped Codex HIGH-01 to web (`eaf3f09`): BAMBU_PROCESS_MAP gained `draft_shield → enable_draft_shield`; `exportBambuStudioJSON` filament fan reads env-scaled S-wrapped `fan_min_speed`/`fan_max_speed`; `formatProfileAsText` reads env-scaled; app.js advanced filament panel renders env-scaled with inline prov. New walkthrough block "v1.0.4 P1.5 HIGH-01-export"; 9 cumulative v1.0.4 OK lines; profile-matrix-audit 55/55 curated + 47196 broad / 0 failures; validate-data clean. iOS untouched (`eeb2915`, Phase 2 gated). **Next step:** S7-2 cold-starts on HIGH-02 — `safe_chamber_temp_max` for PLA-family + material-aware preheat/cold-warning rewrite + harness X1E+PLA assertion flip. Per-finding-checkpoint splits S7 into S7-1/2/3/4 sub-sessions; MEDIUM-02 + OBS-01 deferred to v1.0.5.
...
- **v1.0.4 config-impact implementation — Phase 1 COMPLETE; Phase 1.5 triaged + HIGH-01 shipped.** Setup complete (S1, 2026-05-13): [`merged.md`](../reviews/2026-05-11-config-impact-qa/merged.md) is the owner-accepted queue (locked SHA `5bcd68b`); [`2026-05-13-v1.0.4-config-impact.md`](../superpowers/plans/2026-05-13-v1.0.4-config-impact.md) is the per-task implementation plan. Phase 1 commits: S2 Tasks 1/2/3 (web `133be38` → `b238a82`); S3 Task 4 (`bc070af` + `bf05586`); S4 Task 5 (`dc49c52` + `1695cba`); S5 Tasks 6/7 (`6f9e542` + `901153a`). **Phase 1.5:** S6 prepped audit packet (web `690519e`); owner ran Codex; response captured in S7-1 (`fe2964e`). Codex returned 0 CRITICAL / 2 HIGH / 2 MEDIUM / 1 LOW / 1 OBSERVATION. **Owner-triaged S7 scope (locked):** HIGH-01 + HIGH-02 + MEDIUM-01 + LOW-01 in v1.0.4; defer MEDIUM-02 + OBSERVATION-01 to v1.0.5. HIGH-02 modeling: Option B (data + engine + harness + material-aware cold-env warning copy). **S7-1 shipped Codex HIGH-01 (`eaf3f09`):** BAMBU_PROCESS_MAP gained `draft_shield → enable_draft_shield`; `exportBambuStudioJSON` filament fan reads env-scaled S-wrapped fields; `formatProfileAsText` + `app.js` advanced filament render switched to env-scaled. New walkthrough block "v1.0.4 P1.5 HIGH-01-export"; 9 cumulative v1.0.4 OK lines; matrix-audit + validate-data clean. **Next step:** S7-2 cold-starts on HIGH-02 (`safe_chamber_temp_max` for PLA-family + material-aware preheat/cold-env copy + harness X1E+PLA assertion flip). Then S7-3 MEDIUM-01 (bed-clamp attribution), then S7-4 LOW-01 (negative assertions). Per-finding checkpoint + Trigger A between sessions; iOS push gate enforced. **After Phase 1.5:** Phase 2.1 (Task 8 — iOS engine/data sync + XCTest + iOS local commit) → Phase 2.2 (Task 9 — UI walkthrough + MARKETING_VERSION bump + iOS local commit + 5-point ship-ready handoff). Owner manually dispatches TestFlight after Phase 2.2.
