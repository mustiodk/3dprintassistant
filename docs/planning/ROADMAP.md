...
- **Printer Intake Automation** — turn missing-printer Discord feedback into a reviewed, shippable printer add. Designed 2026-06-12 via ai-om Agent Factory; spec = two contracts at [`../../../ai-operating-model/docs/agents/printer-intake-scout.md`](../../../ai-operating-model/docs/agents/printer-intake-scout.md) + `printer-addition-assistant.md`. **Phase 1 (tee) + Phase 2 (Scout) SHIPPED 2026-06-13; tee activation fixed same day:** `functions/api/feedback.js` tees `missingPrinter` to remote KV namespace `PRINTER_INTAKE` (`f3d89a4e…`, bound in `wrangler.toml`); `scripts/printer-intake-scout.js` is the deterministic triage core — built, 28-test suite, dual-hostile-reviewed (sub-agent + bridge→Codex, 4 patch rounds), QA'd (43 scenarios → 38/43, 0 safety issues), and now reads live KV via `wrangler kv ... --remote` (TDD regression added). Root cause of the apparent tee blocker was the diagnostic/read path querying local/default Wrangler KV, not a Worker write failure; `wrangler tail` showed `env.PRINTER_INTAKE` present and `putOk:true`. Smoke keys were deleted; live queue verified empty. **Next:** researcher rehearsal (Printer Addition Assistant dry-run on Voxelab Aries + adversarial cases). Ship-path web-only via `data/printers.json` + additive iOS overlay; no TestFlight/app-version step for printer adds. FDM-only Phase 1.0 governs (resin auto-declined). `[Web + automation]`
