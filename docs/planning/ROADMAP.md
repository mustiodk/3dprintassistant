...
**Last updated:** 2026-05-11 — Configuration-impact QA pass landed both deliverables: Claude pass (29 findings) and Codex pass (10 findings) at `docs/reviews/2026-05-11-config-impact-qa/{claude,codex}.md`. 5 mutual HIGH findings form the safe core of the future v1.0.4 batch (per Claude's adjudicated cross-pass review section in `claude.md`). Codex cross-pass review of Claude is the next step — prompt ready, owner-triggered. Earlier this session: web-only fix `809549d` shipped (Advanced filament temps always render explicit Initial + Other layer rows; parity with iOS `3a59cd1`). v1.0.3 still in App Review with build `202605101637`; stale builds `202605090842`, `202605101130`, `202605101544` must not be submitted.
...
- **Configuration-impact QA pass — cross-pass review cycle in progress.** Both independent passes landed (Claude: 29 findings; Codex: 10 findings) at `docs/reviews/2026-05-11-config-impact-qa/{claude,codex}.md`. Claude's cross-pass review section in `claude.md` adjudicates 5 mutual HIGHs as the v1.0.4 safe core. **Next step:** Codex cross-pass review of Claude (prompt prepared in 2026-05-11 session log; owner-triggered). After Codex's cross-pass section lands, owner produces `merged.md` in the same directory. Drives a v1.0.4 batch.
