...
- **Feedback-pipeline evolution (5 specs, S1→S5)** — turn the printer-intake pipeline from
  ...
  **ALL 5 specs + 5 plans now QA-green** (each run write→sub-agent-review→patch→QA, autonomous 2026-06-15;
  ...
  - **S1** iOS contextual-feedback prefill fix — `.sheet(item:)` migration (8 sites/6 screens) so
    "Printer missing?" etc. open the correct category; fixes the iOS funnel feeding the Scout. iOS,
    push-gated, single commit, no-local-test (CI/full-Xcode). **Spec + impl plan QA-green**
...
| P2 | **Overlay collision hardening (2026-06-14):** drop the all-or-nothing disjoint *guard* in `PrinterCatalogProvider.validatePayload` so a stale/colliding overlay entry is skipped (via `mergedArray` override-by-id) instead of dropping the **whole** overlay; add an XCTest. Prevents recurrence of the Aries-invisible bug when a future binary bakes in an overlay printer. Needs an iOS binary (push gate + real XCTest run). Spec Part C: [`../superpowers/specs/2026-06-14-ios-overlay-aries-collision-fix-design.md`](../superpowers/specs/2026-06-14-ios-overlay-aries-collision-fix-design.md). | iOS |
