...
| **Phase DQ — Data quality & pro-relevance** | 🟩 **2/5 shipped** — DQ-1 (provenance) ✅ 2026-04-24 AM + DQ-1-followup (filament panel) ✅ 2026-04-24 PM; DQ-2 (Safe/Tuned) ✅ 2026-04-24 PM; DQ-3 deferred post-v1.0.3 | — | 2–3 sessions |
| **v1.0.3 batch** | 🟩 **2/5 shipped** — item 1 (missing printers: Kobra X correction + Centauri Carbon) ✅ 2026-05-08; item 3 (iOS App Store review prompt) ✅ 2026-05-08 (TestFlight dispatched); items 2/4/5 pending | — | varies |

---

## v1.0.3 batch

5 user-facing items shipped across web + iOS. Owner pivoted from DQ-3 to this batch on 2026-05-08.

| # | Item | Platform | Status |
|---|---|---|---|
| 1 | Missing printers (Anycubic Kobra X correction + Elegoo Centauri Carbon) | Web + iOS | ✅ **SHIPPED 2026-05-08**. Kobra X corrected (6 fields: open-frame bedslinger, not enclosed CoreXY — `series` / `series_group` / `enclosure` / `max_bed_temp` / `available_nozzle_sizes` / `open_door_threshold_bed_temp` + notes rewrite) — web `0f47b44`, iOS `b0d1315`. Centauri Carbon added (CoreXY, passive enclosure, 320°C nozzle, 110°C bed, 500 mm/s peak, 32 mm³/s flow, 121-point auto-leveling) — web `8de9381`, iOS `15930c1`. Both verified verbatim against primary sources (Anycubic FAQ + Elegoo product page in `Obsidian Vault/50-Wiki/raw/3dpa/`). 4 commits per one-finding-one-commit-per-platform rule. Web auto-deployed via Cloudflare Pages; iOS picks up in next TestFlight build. Walkthrough 10/10 + DQ-2 3/3 green; iOS XCTest 64/64 green. 3 design choices captured in [`docs/research/gemini-printer-specs-kobra-audit-and-centauri.md`](../research/gemini-printer-specs-kobra-audit-and-centauri.md) § "Locked Phase A scope": Centauri Carbon `multi_color_systems: []` (CANVAS still on Elegoo's roadmap); Kobra X `available_nozzle_sizes: [0.4, 0.6, 0.8]` (0.25 deferred IR-5); Centauri Carbon `available_plates: ["textured_pei"]` (PLA-specific plate id deferred IR-5). |
| 2 | Expanded environments | Web + iOS | ⏳ Gemini handover #2 drafted at [`docs/research/gemini-environments-taxonomy-research.md`](../research/gemini-environments-taxonomy-research.md); not yet sent to Gemini. |
| 3 | iOS App Store review prompt | iOS | ✅ **SHIPPED 2026-05-08** — 6 Codex peer-review passes (1 design + 4 milestone + 1 post-implementation) + 1 Gemini research pass on [`docs/research/gemini-ios-review-prompt-best-practices.md`](../research/gemini-ios-review-prompt-best-practices.md). Design packet at [`codex/ios-review-prompt/codex-2026-05-08-ios-review-prompt-packet.md`](../../codex/ios-review-prompt/codex-2026-05-08-ios-review-prompt-packet.md); post-impl review at [`codex/ios-review-prompt/codex-2026-05-08-ios-review-prompt-implementation-review.md`](../../codex/ios-review-prompt/codex-2026-05-08-ios-review-prompt-implementation-review.md). 5 caught-before-ship bugs (4 design + 1 runtime stranded-eligibility). 4 iOS commits (`756b107` + `1bb543e` + `de4aa9d` + `2309af1`) + version bump (`066add8`). 64/64 XCTest green. TestFlight dispatched run [25557115706](https://github.com/mustiodk/3dprintassistant-ios/actions/runs/25557115706). Eligibility heuristic anchored to Apple's "Requesting App Store Reviews" sample (`>= 4 distinct successful profile generations`); `ReviewPromptService` + `StoreKitDistributionDetector` + `ProfileKeyHasher`; OutputView surface + 2-second Apple-prescribed delay. **Note:** the design + review packets live on the `ai/operating-model-pilot` branch; the actual iOS code lives on iOS `main`. |
| 4 | Analytics | Web + iOS | ⏳ Gemini handover #4 not drafted. |
| 5 | Web output-panel UX | Web | ⏳ Not started. |

**IR-5 followups filed by item 1:**

- **`[0.25mm-nozzle-catalog]`** — Anycubic offers 0.25mm nozzles for Kobra X (likely usable on other Anycubic Kobras with same hot-end). Adding 0.25 to project requires: new `data/nozzles.json` entry with material compatibility list, locale strings (en + da), engine compatibility logic, and a sweep of `available_nozzle_sizes` across all 65 existing printers to identify which support it. Project-wide schema expansion. Filed 2026-05-08.
- **`[pla-specific-plate-id]`** — Centauri Carbon ships a dual-sided plate (Textured PEI + a PLA-specific surface). Adding a new `pla_specific` plate identifier requires: plate-catalog expansion, locale strings, engine compatibility audit (does any warning/checklist key off plate type?). Project-wide schema expansion. Filed 2026-05-08.
