...
**Last updated:** 2026-04-19 — **Web warnings bugfix shipped** (`82e10ac`). RB-1 engine-side conversion to structured warning objects was never committed back in April, so production rendered "undefined" for every warning card. Live now. Follow-up diagnosed: configurator's `0.30mm` Draft layer height exceeds Bambu's printer-level `0.28mm` ceiling for 0.4mm nozzles — Bambu Studio rejects the import. Fix queued (data-only, next session). iOS EU distribution still blocked on DSA Trader Status verification (submitted 2026-04-16, awaiting Apple).
...
| #039 | Printer-capability clamping + slicer-aware values | Medium (~7 h, one focused session) | Full spec: [`docs/specs/IMPL-039-preset-clamping.md`](../specs/IMPL-039-preset-clamping.md). Audit after the 0.30→0.28 Draft-layer bug found **2 HIGH risks** (support_interface_pattern mismatch with Bambu's `rectilinear_interlaced`; hardcoded `initial_layer_height: "0.20 mm"` exceeds max on 0.2mm nozzle — same class as the fixed 0.30 bug) and **5 MEDIUM risks** (infill pattern names not universal across slicers; speed caps ignore `printer.max_speed`; line widths don't scale for 0.2/0.8mm; retraction is material-only not nozzle/extruder-aware). Plan: add `limits` block to `printers.json` per nozzle, introduce `clampEmit()` helper + `slicer_capabilities.json` mapping, route every hardcoded threshold through them, replace UI-text pattern names with slicer-aware lookups. Every fix verified against the `bambu configs/*.json` vendor presets already in the repo. Web + iOS execute in one pass — engine + data are shared; iOS needs one new file registration in `EngineService.buildFetchData()`. |
|---|---------|-------|-------|
