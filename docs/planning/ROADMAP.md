...
- **Resin-scaling discovery (PoC, docs-only)** — formally started 2026-05-17 as a multi-goal Proof of Concept: (1) substantive resin sub-project discovery producing decision-grade material, (2) testing bridge as a multi-round quality amplifier, (3) honest autonomy ceiling probe. Resin product artifacts live at `docs/resin-scaling/` (problem-statement v3 + parked survey v1 to date); PoC meta-track lives at `../../../ai-operating-model/docs/autonomy-poc-2026-05-resin/` (charter, scorecard, bridge-rounds, owner-asks). **Audience is the owner, not 3dpa users** — internal hobby exploration, no public surface changes. Gate 1 (wizard-frame fit, desk-researchable) is the primary blocker; Gate 2 (owner-preference success target with 7-option spectrum A–G) follows. Off-limits during discovery: live engine / data / UI changes to FDM product. Next: bridge round 2 on v3 + technical-differences.md drafting (Gate 1). `[Web docs only]`
