...
- **Printer Intake Automation** — turn missing-printer Discord feedback into a reviewed, shippable printer add. Designed 2026-06-12 via ai-om Agent Factory; spec = two contracts at [`../../../ai-operating-model/docs/agents/printer-intake-scout.md`](../../../ai-operating-model/docs/agents/printer-intake-scout.md) + `printer-addition-assistant.md`. **Phase 1 done (local-only):** `functions/api/feedback.js` tees `missingPrinter` submissions to a `PRINTER_INTAKE` KV namespace — fail-open + dormant until the wrangler binding lands (commit `2995ece`, **intentionally unpushed — owner: no deploy yet**). **Phase 2 next:** build `scripts/printer-intake-scout.js` (deterministic triage/dedupe/FDM-decline → candidate packets) against fixtures — see [`../sessions/NEXT-SESSION.md`](../sessions/NEXT-SESSION.md). Ship-path is web-only (printers.json + overlay → Cloudflare); no app-version/TestFlight for printer adds. Owner infra to activate Phase 1: `wrangler kv namespace create PRINTER_INTAKE` + scoped CF token + web push. The Scout half-automates [`../runbooks/printer-addition-protocol.md`](../runbooks/printer-addition-protocol.md); FDM-only Phase 1.0 still governs (resin auto-declined). `[Web + automation]`
