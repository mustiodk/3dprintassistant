...
**Last updated:** 2026-05-13 — v1.0.3 is live on the App Store; remote overlay `content_version=2026051202` ships i7 under Creality / i Series for current iOS clients. v1.0.4 S2 (impl) complete: web `b238a82` ships Tasks 1, 2, 3 (strength `speed_multiplier`, env data layer + cold-warning fix + env clamp attribution, physical printer × nozzle guard). 4 cumulative v1.0.4 walkthrough OK lines; 47196/0 broad audit. iOS untouched (Phase 2 gated). **Next step:** S3 cold-starts and executes Task 4 (physical printer × plate guard + material plate range).
...
- **v1.0.4 config-impact implementation.** Setup complete (S1, 2026-05-13): [`merged.md`](../reviews/2026-05-11-config-impact-qa/merged.md) is the owner-accepted queue (locked SHA `5bcd68b`); [`2026-05-13-v1.0.4-config-impact.md`](../superpowers/plans/2026-05-13-v1.0.4-config-impact.md) is the per-task implementation plan (TDD-first per finding, 7 web tasks + 1 iOS sync task). Owner-locked planning defaults are reproduced verbatim in `merged.md`. S2 (2026-05-13) shipped Tasks 1, 2, 3 (web `133be38` → `b238a82`): strength `speed_multiplier`, env data layer + cold-warning copy + env clamp attribution, physical printer × nozzle guard. Walkthrough now has 4 cumulative v1.0.4 OK lines; profile-matrix-audit clean across 47196 broad configs. **Next step:** S3 cold-starts and executes Task 4 (physical printer × plate guard + material plate range). Multi-session autonomous arc; per-finding checkpoint rule + Trigger A close between sessions. iOS push gate enforced — local commits only until 5-point ship-ready check passes.
