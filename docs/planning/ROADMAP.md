...
**Last updated:** 2026-05-13 — v1.0.3 is live on the App Store; remote overlay `content_version=2026051202` ships i7 under Creality / i Series for current iOS clients. **v1.0.4 Phase 1.5 — HIGH-01 + HIGH-02 shipped web-only (S7-1 + S7-2).** Codex audit response captured at `codex/v1.0.4-audit/codex-2026-05-13-v1.0.4-audit-response.md` (commit `fe2964e`); triage returned 0 CRITICAL / 2 HIGH / 2 MEDIUM / 1 LOW / 1 OBSERVATION. Owner-approved scope: HIGH-01 + HIGH-02 + MEDIUM-01 + LOW-01 in v1.0.4; defer MEDIUM-02 + OBSERVATION-01 to v1.0.5. **S7-2 shipped Codex HIGH-02 (Option B) to web in two commits:** `a4b8873` (impl) + `f2aa84f` (tightening after internal code-review subagent flagged PVA scope creep + vcold harness gap + validate-data schema gap). Data: `safe_chamber_temp_max: 50` added inside `enclosure_behavior` on all 5 PLA-family materials; existing `chamber_above_material_safe` guard at engine.js:1746-1754 now fires for X1E+PLA naturally. Engine: `pla_heat_creep` extended from `enclosure === 'passive'` to `enclosure !== 'none'`; `getChecklist` "Preheat enclosure" suppressed for PLA-family + enclosed; `getWarnings` env-verbatim pass filters `/door closed|preheat the enclosure|preheat \(/i` for PLA-family + enclosed. All three suppression predicates gated on `material.group === 'PLA'` for consistency with pla_heat_creep. Tightening also added optional-field range checks in validate-data.js for `safe_chamber_temp_max` + `open_door_threshold_bed_temp`. New "v1.0.4 P1.5 HIGH-02" walkthrough block (8 sub-asserts: cold + vcold + passive non-regression + PETG non-regression + PVA scope guard); 10 cumulative v1.0.4 OK lines; matrix-audit 47196 broad / 0 failures + 0 curated; validate-data clean. iOS untouched (`eeb2915`, Phase 2 gated). **Next step:** S7-3 cold-starts on MEDIUM-01 — first-layer bed-clamp attribution honesty (engine.js:1227 `env.bed_first_layer_adj` + :1624-1631 warning copy + :1706-1727 bed-clamp warning attribution discipline).
