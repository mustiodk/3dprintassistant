...
| #039 | Printer-capability clamping in `resolveProfile()` | Medium (2–4 h data + small engine refactor) | Today's `0.30 → 0.28` Draft-layer bug surfaced a structural gap: the engine emits hardcoded values (`nozzle.size * 0.75`, fixed speeds, temps) that don't always match each printer's actual slicer-preset range. **Plan:** (1) add `limits` block to every printer in `data/printers.json`, keyed by nozzle size — `max_layer_height`, `min_layer_height`, `max_print_speed`, `max_nozzle_temp`, `max_line_width`. Source of truth: vendor preset JSONs (samples already in `bambu configs/`). (2) add a `clampToLimit(value, limit, warningId, state, warnings)` helper in engine. (3) route every `Math.min` / 75%-style threshold through the helper. (4) when a clamp fires, emit a structured warning explaining the cap. Covers Bambu + Prusa + Orca + Creality with one data model. Blocked by nothing — can ship in one focused session. |
|---|---------|-------|-------|
