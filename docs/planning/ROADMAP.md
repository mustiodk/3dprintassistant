...
**Last updated:** 2026-05-13 — v1.0.3 is live on the App Store; remote overlay `content_version=2026051202` ships i7 under Creality / i Series for current iOS clients. **v1.0.4 Phase 1 is COMPLETE on web (7/7 tasks shipped).** S5 shipped Tasks 6 + 7 in one session with zero reviewer round-trips: web `6f9e542` (Task 6 — chamber safe-cap guard, warning-only `chamber_above_material_safe` adjacent to the existing PA/PC active-chamber tip; HIGH-05) + web `901153a` (Task 7 — `cf_small_nozzle` retired, `nozzle_too_small` → `nozzle_below_min_diameter` with parameterized body, nozzle-side `suitable_for`/`not_suitable_for` arrays + schema soft-check + validate-data array checks dropped per owner default 5; HIGH-12 + HIGH-06). Walkthrough now has 8 cumulative v1.0.4 OK lines; profile-matrix-audit 55/55 curated + 0 broad failures at every commit. iOS untouched (Phase 2 gated). **Next step:** S6 cold-starts Phase 1.5 — generates the Codex audit packet (owner-gated dispatch — Option B low-touch) and stops; owner runs Codex manually; S7 cold-starts on the response.
...
- **v1.0.4 config-impact implementation — Phase 1 COMPLETE (7/7 web tasks shipped).** Setup complete (S1, 2026-05-13): [`merged.md`](../reviews/2026-05-11-config-impact-qa/merged.md) is the owner-accepted queue (locked SHA `5bcd68b`); [`2026-05-13-v1.0.4-config-impact.md`](../superpowers/plans/2026-05-13-v1.0.4-config-impact.md) is the per-task implementation plan (TDD-first per finding, 7 web tasks + 1 Codex audit gate + 2 iOS phases). S2 shipped Tasks 1, 2, 3 (web `133be38` → `b238a82`); S3 shipped Task 4 (web `bc070af` + `bf05586`); S4 shipped Task 5 (web `dc49c52` + `1695cba`). **S5 (2026-05-13) shipped Tasks 6 + 7** — web `6f9e542` (Task 6, HIGH-05 chamber safe-cap guard adjacent to active_chamber_heating block; warning-only `chamber_above_material_safe` fires on PETG/PET-group materials carrying `enclosure_behavior.safe_chamber_temp_max=50` when `printer.active_chamber_heating=true`; no profile field) + web `901153a` (Task 7, HIGH-12 + HIGH-06 — retired `cf_small_nozzle`, renamed `nozzle_too_small` → `nozzle_below_min_diameter` with parameterized body, removed nozzle-side `suitable_for`/`not_suitable_for` arrays from all 9 nozzles + engine schema soft-check + validate-data array-type checks; `profile-matrix-audit.js:280` updated in same commit per S4 sweep). Walkthrough now has 8 cumulative v1.0.4 OK lines; profile-matrix-audit 55/55 curated + 0 broad failures at every commit. iOS untouched. **Next step:** S6 cold-starts Phase 1.5 — generates `codex/v1.0.4-audit/v1.0.4-commit-range.diff` (from `git diff 5bcd68b..901153a` filtered to engine.js + data/ + scripts/walkthrough-harness.js + scripts/profile-matrix-audit.js), authors the Codex packet at `codex/v1.0.4-audit/codex-YYYY-MM-DD-v1.0.4-audit-packet.md`, pre-creates the empty response file, surfaces a copy-paste-ready dispatch command in the Trigger A close note, and stops. **Owner-gated:** owner dispatches Codex manually. S7 cold-starts on the response file. Stop conditions: ≥1 CRITICAL or ≥5 HIGH → owner pauses; 1-4 HIGH → autonomous web-only remediation; MEDIUM/LOW → owner-triaged. Green-path collapse: 0 findings → S7 compresses into a confirmation note at the start of S8. Multi-session autonomous arc; per-finding checkpoint + Trigger A between sessions; iOS push gate enforced. **After Phase 1.5:** Phase 2.1 (Task 8 — iOS engine/data sync + XCTest + iOS local commit) → Phase 2.2 (Task 9 — UI walkthrough + MARKETING_VERSION bump + iOS local commit + 5-point ship-ready handoff). Owner manually dispatches TestFlight after Phase 2.2.
