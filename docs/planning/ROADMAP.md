...
- **Feedback-pipeline evolution (5 specs, S1→S5)** — turn the printer-intake pipeline from
  ...
  one spec + impl plan per topic, each gated spec→review→patch→QA→plan→review→patch→QA with an owner
  check-in between topics; **execution deferred to explicit owner command.**
  - **S1** iOS contextual-feedback prefill fix — `.sheet(item:)` migration (8 sites/6 screens) so
    ...
    push-gated. **Spec QA-green** ([spec](../superpowers/specs/2026-06-15-ios-feedback-prefill-fix-design.md));
    **impl plan = locked next.** `[iOS]`
