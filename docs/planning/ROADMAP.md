...
**Last updated:** 2026-05-13 — v1.0.3 is live on the App Store; remote overlay `content_version=2026051202` ships i7 under Creality / i Series for current iOS clients. v1.0.4 setup session (S1) complete: `docs/reviews/2026-05-11-config-impact-qa/merged.md` locked (web `05e85bd`); per-task implementation plan written at `docs/superpowers/plans/2026-05-13-v1.0.4-config-impact.md` (web `129d6a7`); CR3-M1 audit-script `userLevel` fix shipped (web `8ff25eb`). **Next step:** S2 cold-starts and executes the plan starting at Task 1 (strength `speed_multiplier`).
...
- **v1.0.4 config-impact implementation.** Setup complete (S1, 2026-05-13): [`merged.md`](../reviews/2026-05-11-config-impact-qa/merged.md) is the owner-accepted queue (locked SHA `5bcd68b`); [`2026-05-13-v1.0.4-config-impact.md`](../superpowers/plans/2026-05-13-v1.0.4-config-impact.md) is the per-task implementation plan (TDD-first per finding, 7 web tasks + 1 iOS sync task). Owner-locked planning defaults are reproduced verbatim in `merged.md`. **Next step:** S2 cold-starts and executes Task 1 (strength `speed_multiplier` wired into wall speeds). Multi-session autonomous arc; per-finding checkpoint rule + Trigger A close between sessions. iOS push gate enforced — local commits only until 5-point ship-ready check passes.
