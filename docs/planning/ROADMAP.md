...
### IR-2a — iOS v1.0.2 ship pass ⭐ SUBMITTED TO APP REVIEW 2026-04-23 (awaiting Apple)
...
- [ ] When Apple approves → manually tap **Release This Version** in ASC. Decide EU inclusion based on DSA Trader Status state at that moment — if cleared, enable EU in the rollout; otherwise ship to same ~121 non-EU countries as v1.0.0. `[You]`
...
- [ ] **[DQ-1-followup]** Extend provenance to the filament panel. `getAdvancedFilamentSettings` + `getAdjustedTemps` + `getFilamentProfile` need to emit `prov` tags on their numeric fields (bed_temp, nozzle_temp, fan speeds, max_volumetric_speed, retraction, pressure_advance). Then apply the same ⓘ icon pattern in `renderFilamentPanel` (web) + `FilamentSettingsView` (iOS). DQ-1's `resolveProfile` path is complete — this extends provenance coverage to the other engine output surfaces. Estimated ~2h. Filed 2026-04-24. `[Web+iOS]`
- [ ] **[LOW-003-followup-label]** `renderFilamentPanel` in `app.js:1187` reads `adv.retraction_length` — key was renamed to `retraction_distance` during LOW-003 (shipped 2026-04-23 IR-4 bundle, web `b58b8e2`). User sees "Retraction length undefined" in the Filament Settings panel under SETTING OVERRIDES. Fix: swap the field access + update the locale label key if needed. 2-line fix. Filed 2026-04-24 during DQ-1 commit 4 review. `[Web]`
...
| IR-2a — iOS v1.0.2 ship pass ⭐ | ⏳ **SUBMITTED TO APP REVIEW 2026-04-23.** Remaining: Apple approval + manual release + day-1 monitoring | — | — |
...
| **Phase DQ — Data quality & pro-relevance** | 🟩 **1/5 shipped** — DQ-1 (provenance) ✅ 2026-04-24; DQ-2 next | — | 2–4 sessions |
...
**Progress:** `[🟩⬜⬜⬜⬜ 20%]` — **DQ-1 shipped 2026-04-24**. 4 sub-phases remaining.
