...
| Gate | Scope | Exit criteria | Status | Commit |
|---|---|---|---|---|
| **A-P0** | `scripts/export-audit.js` diffing generated exports vs golden fixtures (Bambu + Prusa; Orca pending) and vs resolved-profile values; resolve contested claims programmatically (zig-zag, BS version, inherits parents, only_one_wall_top). Findings table appended to IMPL-043 §1.4. NO product code. | Harness + findings committed. | [x] | (this commit) |
| **A-P1a** | Data: 5 canonical fields in `objective_profiles.json` (seam_position, only_one_wall_top, ironing_type, ironing_pattern, internal_solid_infill_pattern). | validate-data extended + green; golden deltas enumerated. | [x] | `75e67b1` (with A-P1b — the engine reads and the data land together or neither works) |
| **A-P1b** | Engine: resolveProfile attaches `_slicer_value` sidecars + derives display labels from canonical values. | Walkthrough + matrix-audit green (label deltas enumerated); golden regen enumerated. | [x] | `75e67b1` |
| **A-P1c** | Engine: export passthrough via `_slicer_value`/`mapForSlicer`; `USE_LEGACY_EXPORT` constant (default false) keeps old regex path as instant fallback; drift-guard assertions (exported == resolved) per param. | export-audit green; drift guards green. | [x] | `e880574` |
| **A-P1d** | Defect fixes, one commit each: HIGH-001 scaled retraction (RED first), ironing type/pattern split, support_style 5-option map, version→module constant (per A-P0 findings). | Each RED→GREEN; all harnesses green. | [x] | `a0f3dcf` · `fe8fb02` · `50fc57b` · `62f7a23` |
...
- 2026-07-06 **A-P0 done**: `scripts/export-audit.js` (walkthrough loading pattern; FAIL/warn/info classes; exit 1 while any FAIL lives — it currently exits 1 BY DESIGN, documenting the live defects that A-P1 fixes). Run on pre-P1 code: **3 FAIL** (filament version 2.5.0.14≠golden 2.5.0.18 · HIGH-001 retraction exports 0.6 raw vs 0.4 resolved on the 0.2-nozzle case · ironing_pattern absent) / **1 warn** (golden writes 2-element per-extruder-variant arrays — NEW BS 2.5 schema discovery, owner import test adjudicates 1-element acceptance) / **5 info**. Contested claims resolved empirically — full table at IMPL-043 §1.4b: zig-zag VALID (IMPL-036 wrong; corollary: BS internal-solid has NO "monotonic" → canonical values must route through mapForSlicer), process version correct/filament wrong, process inherits exact match, cross-printer filament parents legitimized by the golden itself, string-bool form confirmed. Prusa fixture parses (373 keys, presets match versions.md) — inventory for Phase 4, no serializer. 9 of 9 value-drift guards on the 0.4 default state PASS (the current export's numeric emissions agree with resolved values on the happy path — the drift is in the scaled/derived cases, exactly where IMPL-036 predicted).
- 2026-07-06 **A-P1 done** (6 commits): a+b `75e67b1` (5 canonical data fields mirroring current behavior EXACTLY — incl. preserving the fine+maximum-only only_one_wall_top quirk; retired dead ironing/seam_aligned flags; validate-data enums; slicerValueFor() + sv() sidecars at every string/boolean/pattern emission; matrix-audit stripped hash UNCHANGED = display byte-identical; golden diff = 391 lines ALL sidecar additions, 0 value changes). c `e880574` (passthrough active, legacy frozen verbatim behind USE_LEGACY_EXPORT=false; enumerated payload delta vs legacy across 4 representative states = EXACTLY ONE: internal_solid 'zig-zag'→'rectilinear', display/export semantic split closed). d1 `a0f3dcf` HIGH-001 (RED = A-P0 audit FAIL 0.6-vs-0.4; now scaled sv). d2 `fe8fb02` ironing_pattern exported (display-side param split deferred to Phase 2 as cosmetic — DECISION: P1 owns export correctness, the UI keys stay stable this train). d3 `50fc57b` support_style 5-map (+ fixed a G0 snapshot coverage hole: support variant used invalid id 'organic'→'easy'; golden now exercises the support block — 'Tree Strong' visible). d4 `62f7a23` BAMBU_PROCESS_VERSION/BAMBU_FILAMENT_VERSION (F2). **export-audit: 0 FAIL / 1 warn** (dual-extruder arrays = owner import test). New walkthrough baseline (sidecar keys in JSON appendix): `062a03ddebd0150b618d4338f7be78fef1ba2aa3afe30304760a23a6ac8fd0dd`. **Web+iOS eval:** web UI unchanged (export buttons already live for Bambu; payload-only fixes); iOS = byte mirror at A-iOS, export UI stays hidden.
