...
| **W5** | Hygiene ride-alongs, one commit each: (a) add Danish keys `secNozzleTemp_prusaslicer` + `secBedTemp_prusaslicer`; (b) fix stale ROADMAP export status row (export is live for Bambu behind Beta, not disabled); (c) align public printer-count copy with count computed from `data/printers.json`. | validate-data green; three separate commits. Pushed. | [x] | (a) `8b6ac5b` — en/da key parity now 274=274; (b) `27062ac`; (c) `97b3127` — data has 74 printers/14 brands, About says 74/14 (en+da), meta rounds to 70+ |
| **W6** | Web ship verification. Confirm all web pushed; `curl` live site + one share URL; 200 + app shell loads. | Post-deploy evidence recorded in Notes. | [x] | (verify-only; evidence in Notes) |
...
| **I2** | iOS Workshop W1 (SwiftUI): saved-profiles shelf; JSON backup document **byte-compatible** with web format (test this). | XCTest green incl. cross-format test. Local commit. | [x] | iOS local `fdd99d4` — WorkshopStore + AppStateWebCodec (persistence refactored onto it) + WorkshopView/Home entry/Output star; byte-compat pinned by a fixture generated from the real web workshop-store.js; 125 unit green |
...
- 2026-07-05 **W6 post-deploy evidence** (~90s after final push `a311730`): `curl https://3dprintassistant.com/` → 200; share URL `/?p=a1&m=petg_basic&n=std_0.4&sq=fine&uc=functional,large` → 200; live HTML contains the new `<title>` ("70+ 3D Printers"), `navWorkshop`, and script tags for `state-codec.js` + `workshop-store.js`; both new assets serve 200. Delivered, not just published. **All web gates W0–W6 green.**
- 2026-07-06 **I0 baseline** (session crossed midnight; web gates were 2026-07-05): iOS repo clean at `518f781` = `origin/main`, `MARKETING_VERSION = 1.0.5` (project.yml + both pbxproj configs) → **this train bumps to 1.0.6 at I4**. This machine has Xcode 26.3 (not the mac-mini's 26.5); `Config.xcconfig` was missing → recreated from `Config.xcconfig.template` with empty values (gitignored, secrets not needed for sim tests). Baseline XCTest: **112 unit tests, 0 failures** (`-only-testing:3DPrintAssistantTests`, iPhone 17 sim `D4B94D7D`, `CODE_SIGNING_ALLOWED=NO`). AppState: `@Observable`, goal fields have non-nil defaults (unlike web's nulls), plus UI-only `brand`; no persistence anywhere (confirmed). App boot: `PrintAssistantApp.task` → PrinterCatalogProvider → DataService.reload → EngineService.initialize; `ContentView` owns `AppState()` + router. New Swift files require `xcodegen generate` (project.yml globs).
