...
- User identity per spec §4.3: strictly `(provider, sub)` — email is non-unique metadata and never auto-links; explicit authenticated link flow (`/api/v1/auth/:provider/start?link=1` while signed in) attaches a second provider to the current user; Apple grants stored per-client in `provider_grants`.
