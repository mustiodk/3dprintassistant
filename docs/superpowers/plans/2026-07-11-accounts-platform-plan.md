...
- `[env.staging]` in `wrangler.toml`: staging Worker on `accounts-staging.3dprintassistant.com` (registered at AP0) with its own D1 + KV + secrets — the executable home of all pre-AP9 real-provider verification.
