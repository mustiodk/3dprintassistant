...
- **Estimate honesty:** AP2 is 2–3 sessions (two providers, ES256 minting, JWKS caching, dual-mode callback, intermediate codes, the two-phase link protocol, rotation-with-grace, ~25-case test matrix, plus the cross-model security loop). If a session boundary demands a split, AP2a (core web sign-in) / AP2b (native + link protocol) become two FORMAL gates: each its own branch + PR + review + ledger row, AP2b strictly after AP2a, and the dark-live flag flip is AP2b's exit step (rule 1 stays intact).
