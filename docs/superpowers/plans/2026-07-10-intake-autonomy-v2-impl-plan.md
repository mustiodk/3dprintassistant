...
- [ ] **B5.0 pre-drill snapshot (explicit; Codex SF-2):** `node scripts/republish-overlay.js --snapshot scripts/.intake-runner-state/pre-b5-latency-known-good.json` — this exact path is the drill's rollback target. *(Path superseded at execution, 2026-07-10: the plan's original `.printer-intake-out/` location is rmSync-wiped by the Scout test suite, which the B5.3 exit checklist itself runs — see gate ledger B1/B5 rows.)*
