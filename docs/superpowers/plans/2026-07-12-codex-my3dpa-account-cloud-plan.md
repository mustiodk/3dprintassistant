...
**Tests:** exact/legacy IDs, crash points, corrupt backup, unknown future fields, repeated migration, quota, signed-out reload, v1↔PDM2 round trip. A clock-injected signed-out cleanup fixture must perform two successful local reopen → decode → invariant-check → round-trip validations separated by a browser restart, prove the backup is ineligible after only one cycle, then prove eligibility and automatic erasure exactly 30 days later plus explicit early-delete/save-copy behavior. Prove a backup is valid only when it is stored at a distinct durable location and passes byte hash plus restore validation; overwriting the source record is not a backup. When File System Access is unavailable, require an explicit JSON download plus user confirmation before destructive migration or first-sync cleanup.
...
| O1 | B1,A1c,S0d | redacted ledger commands: `npm ci`, exact `npx wrangler d1 create --jurisdiction=eu`, migrate/deploy/smoke/rollback invocations | production flags false | prior deploy + checkpoint restore |
