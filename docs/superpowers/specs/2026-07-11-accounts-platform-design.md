...
- **Identity:** a user is identified **strictly by `(provider, sub)` — email never auto-links accounts**, verified or not (recycled/reassigned emails with a fresh provider `sub` would otherwise attach a stranger to the prior user's synced data; Google explicitly warns email must not be used as an identifier). Same email on two providers ⇒ two distinct users by default. Cross-provider linking exists only as an **explicit authenticated link flow**: from My 3dpa, a signed-in user runs "Link Apple/Google", completes that provider's flow in-session, and the new `(provider, sub)` attaches to the current user. `users.email` is plain metadata — nullable and **non-unique**. Tests: same-verified-email two-provider isolation; explicit-link attach; linked-identity sign-in resolves to the same user.
