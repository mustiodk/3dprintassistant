...
- Realtime collaborative sync (sync is coarse-grained versioned-document sync with deterministic client-side merge).

---

## 3. Decisions (APD0–APD15) — owner ratifies before build
...
- **APD13 — Monetization Phase-2 anchor: explicit owner decision required (AP0 blocker).** MD2 (ratified 2026-07-11) anchors the "3dpa Pro" one-time unlock on **filament inventory** and lists **cloud sync** as a Pro candidate. This spec ships inventory (free, local-first) and cloud sync — and MD0 forbids retro-paywalling anything once shipped free. Shipping both free would silently gut Phase 2's entire monetizable surface, irreversibly. The owner must pick ONE before any build:
  ...
  Recommendation: **(b)**. Whatever the pick, it is recorded as an MD2 amendment note in the monetization plan in the same PR that ratifies it. **Executability:** the companion plan as written is executable end-to-end only under option (a). Under (b), (c) or (d) a **Phase-2 Pro companion plan is a prerequisite for the affected gates** — it must add the entitlement schema/API, the IAP purchase rail, an entitlement guard on `/api/v1/sync`, cap enforcement for (d), and (for (b)) reorder the launch so the iOS purchase rail (AP10-side) exists before public sync launch (AP9) — written and reviewed before AP4 starts. Until the owner decides: AP1–AP3 are unaffected under every option; AP7 is unaffected under (a)/(b) but changes scope under (c)/(d); AP4+ is design-final but build-blocked.
...
Cloudflare Workers  /api/v1/auth/*  /api/v1/sync/:doc  /api/v1/account/*
        │
        ▼
      D1 (users, oauth_identities, refresh_tokens, auth_codes, link_intents, devices, sync_docs, revocation_outbox)  +  KV (deletion log)
```
...
CREATE TABLE users (
  ...
  email TEXT,                       -- metadata ONLY, nullable + non-unique; identity is (provider, sub) — email never links accounts (§4.3)
  display_name TEXT,
...
CREATE TABLE oauth_identities (
  ...
  email TEXT,                       -- as asserted by provider at link time (metadata only)
  email_verified INTEGER NOT NULL DEFAULT 0,
  provider_grants TEXT,             -- Apple only; JSON map { client_id: AEAD-encrypted refresh_token } — one Apple identity can hold BOTH the web (Services ID) and native (bundle id) grants, and /auth/revoke requires the client_id that matches each grant
  created_at INTEGER NOT NULL,
  PRIMARY KEY (provider, subject)
);
CREATE TABLE link_intents (         -- single-use native link intents (see §4.3 native link path)
  ...
  pending_identity TEXT,            -- JSON {provider, sub, email, grants}; WRITE-ONCE by the callback; attached ONLY at link-finalize (PKCE + receipt proven)
  callback_receipt TEXT,            -- one-time receipt hash minted by the callback, required at link-finalize
...
CREATE TABLE revocation_outbox (    -- provider revocations that must survive account deletion (see §4.3)
  id TEXT PRIMARY KEY,
  provider TEXT NOT NULL,
  client_id TEXT NOT NULL,          -- the client the grant was issued to (Services ID or bundle id) — /auth/revoke must match it; one row PER grant
...
CREATE TABLE sync_docs (
  ...
  compact_watermark INTEGER NOT NULL DEFAULT 0,  -- highest since_version ever compacted (see §4.4)
...
- **Identity:** a user is identified **strictly by `(provider, sub)` — email never auto-links accounts**, verified or not (recycled/reassigned emails with a fresh provider `sub` would otherwise attach a stranger to the prior user's synced data; Google explicitly warns email must not be used as an identifier). Same email on two providers ⇒ two distinct users by default. Cross-provider linking exists only as an **explicit authenticated link flow**: from My 3dpa, a signed-in user runs "Link Apple/Google" → `/start?link=1` (requires a valid session at start time) **seals the initiating `user_id` + session `jti` into the AEAD pre-auth cookie payload** (the pre-auth cookie is SameSite=None, so it survives Apple's cross-site `form_post` even though the Lax session cookie does not — the link intent must therefore be carried by the pre-auth cookie, never inferred from the session cookie at callback). At callback the server unseals the intent, verifies the bound session is still valid and belongs to the same user, and only then attaches the new `(provider, sub)` to that user. **Native link path (Bearer clients have no cookie context):** the app first calls `POST /api/v1/auth/link-intent` (Bearer-authenticated) which mints a **single-use, 120s, server-stored link intent** (`link_intents`: hashed id, user_id, session jti, provider, client, PKCE challenge, expires_at, consumed_at) and returns its opaque id; the app opens `ASWebAuthenticationSession` at `/start?link_intent=<id>`; **the callback never attaches** — it only stores a *pending identity* against the intent (no mutation; pending records expire with the intent), because until PKCE proof arrives the intent id alone must not be able to change the account. The pending write is **write-once** (a second callback on the same intent is rejected — no overwrite, no race window: the conditional INSERT/UPDATE succeeds exactly once) and the callback additionally mints a one-time **callback receipt** returned to the app via the custom-scheme redirect. The app then calls `POST /api/v1/auth/link-finalize` (Bearer-authenticated, same bound session) supplying the **PKCE verifier AND the callback receipt**; the server atomically consumes intent + pending identity + receipt and attaches `(provider, sub)` to the bound user. A stolen intent id is thus useless (cannot finalize without Bearer session + verifier + receipt), and an attacker-injected callback cannot be finalized by the victim (the victim's app never holds the attacker's receipt; a poisoned second callback is rejected by write-once). Tests: stolen-intent use without finalize (no mutation), attacker-callback→victim-finalize fails (receipt mismatch), concurrent callback race → exactly one write, callback-without-finalize expiry, verifier mismatch, intent replay, expiry, session-invalidation, collision (AP2 + I1/I4). **Collision rule (both paths):** if the `(provider, sub)` already belongs to a *different* user → 409 "already linked to another account", **no mutation** (no merge, no steal); already linked to the *same* user → no-op success. `users.email` is plain metadata — nullable and **non-unique**. Tests: same-verified-email two-provider isolation; explicit-link attach; link intent bound to initiating user (session invalidated/changed between start and callback → rejected, no mutation); already-owned identity → 409 no-mutation; linked-identity sign-in resolves to the same user.
...
- **Apple email caveat:** Apple returns email **only on first authorization** → `users.email` is nullable; account identity never depended on it (identity is `(provider, sub)` per the rule above), so a missing email costs only display metadata.
...
- **Deletion:** `DELETE /api/v1/account` (authenticated) runs in this order — KV and D1 cannot share a transaction, so the KV write comes FIRST and is idempotent: (1) write the **KV deletion intent** (`del:<sha256(user_id)>` → deleted_at; idempotent — re-running deletion for the same user overwrites harmlessly; the restore journal therefore exists even if the D1 batch then fails and is retried), (2) one **atomic child-first D1 batch**: copy **every Apple grant — one outbox row per `(client_id, encrypted token)` pair**, since an identity may hold both the web (Services ID) and native (bundle id) grants and Apple's `/auth/revoke` requires the matching `client_id` — into `revocation_outbox` (the cascade would otherwise destroy the only credentials the mandatory revoke needs), then delete identities, refresh tokens, auth_codes, link_intents, devices, sync docs, then the `users` row (all FKs also declare `ON DELETE CASCADE` as belt-and-braces — D1 enforces foreign keys, so child-first ordering is mandatory, not stylistic). Tests cover: KV-written-but-batch-failed retry, double-delete idempotency. Apple `/auth/revoke` is then attempted from the outbox — success deletes the outbox row; failure leaves it for opportunistic retry (piggybacked on later requests) with a 30-day owner-runbook manual fallback. The user-facing deletion is complete at the batch; revocation is an independent, retryable follow-through. **Bounded exception to "all server rows deleted immediately" (disclosed in the privacy policy):** when the Apple revoke has not yet succeeded, the AEAD-encrypted revocation credential(s) (one per outstanding grant) persist in `revocation_outbox` for ≤30 days, solely to complete Apple's mandatory token revocation, then is removed (on success or via the runbook fallback). No other user data survives the batch. `requireAuth` treats a JWT whose user row no longer exists as unauthenticated. iOS surfaces deletion per 5.1.1(v). **Kill-switch carve-out:** `ACCOUNTS_API_ENABLED=false` disables new sign-ins and sync but `DELETE /api/v1/account`, `GET /api/v1/account/export`, `refresh` (existing sessions), `auth/status`, and `logout` stay reachable — GDPR rights survive the kill switch usably, not just nominally (see §4.5 for the client-side rights UI rule).
...
- **Two flags** (one flag cannot express the dark launch): `ACCOUNTS_API_ENABLED` (Worker **secret**, not a `[vars]` entry — see plan AP1 for why; the **kill switch**; off ⇒ NEW authentication (start/callback/token) and sync return 503, while the **rights carve-out stays functional end-to-end**: `refresh` for existing sessions (refresh is inherently existing-session-only, and without it a <1h access token would strand users out of their own GDPR rights), `auth/status` (returns `{api:false, ui:false, rights:true}`), account export, account delete, logout. Client behavior under kill switch: sign-in and sync UI hidden, but a signed-in user still sees a minimal account-rights section (export / delete / sign out). Test: export + delete succeed after access-token expiry with the kill switch active) and `ACCOUNTS_UI_ENABLED` (Worker **secret**, surfaced to clients via `GET /api/v1/auth/status` → `{api, ui, rights}` (`rights` flips true at AP9's prod launch — when the rights endpoints and the rewritten privacy policy actually exist — and stays true; it signals only that the GDPR carve-out endpoints exist, independent of `api`/`ui`. Production flags stay off entirely until AP9; pre-AP9 verification happens on preview deployments); off ⇒ clients hide all account UI). Dark launch (the short AP9 window, after the policy is live) = API on, UI off. Both off = today's behavior on every **account surface**, byte-for-byte (AP7's local-first inventory is deliberately an unflagged additive free feature per MD0/APD5 — it is independent of the account flags and stays visible signed-out; the byte-for-byte promise is scoped accordingly).
...
|---|---|---|
| P0 | Owner ratification (APD0–APD15) + provider/app registrations + privacy-policy draft | owner |
...
|---|---|---|
| Solo-dev operational burden (auth is a liability surface) | HIGH | No passwords; two OAuth providers only; managed platform (CF); kill switch; rate limits; small API surface; recovery runbook + quarterly D1 export |
...
1. Signed-out web + iOS behave byte-for-byte as today **on account surfaces** (feature-flag-off path proven; the unflagged local-first inventory from P4 is a deliberate additive free feature outside this promise; no engine/data diff in P1–P5 — golden/walkthrough proof per PR).
...
4. My 3dpa page: cloud-account export produces the complete server-held JSON (labeled as such, local backup export still available for device data); delete (incl. Apple token revocation) removes all server rows **or leaves only the documented ≤30-day revocation-outbox credential when the Apple revoke is pending (test asserts exactly these two end states)** — plus re-auth after delete creates a fresh empty account; export/delete/refresh/logout remain reachable AND usable (post-token-expiry test) with the kill switch on.
...
7. All new Workers covered by tests in the `functions/api/**/*.test.mjs` pattern; walkthrough harness + golden snapshots prove engine/data untouched through P5.
