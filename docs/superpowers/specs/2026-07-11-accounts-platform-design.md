...
CREATE TABLE link_intents (         -- single-use native link intents (see §4.3 native link path)
  ...
  pending_identity TEXT,            -- JSON {provider, sub, email, grants} stored by the callback; attached ONLY at link-finalize (PKCE-proven)
