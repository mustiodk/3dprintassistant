...
- **Two flags** (one flag cannot express the dark launch): `ACCOUNTS_API_ENABLED` (Worker env; off ⇒ all `/api/v1` endpoints return 503 **except the three GDPR carve-outs — account delete, export, logout (§4.3)** — this is the **kill switch**) and `ACCOUNTS_UI_ENABLED` (Worker env, surfaced to clients via `GET /api/v1/auth/status` → `{api, ui}`; off ⇒ clients hide all account UI). Dark launch = API on, UI off. Both off = today's behavior, byte-for-byte.
