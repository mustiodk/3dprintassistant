...
A live reconciler scans expired lifecycle leases at least once per minute and also schedules on repeated `lifecycle_in_progress`. It looks up the exact `op_id/lifecycle_generation` in the external ledger: with no `committed_intent`, conditionally delete only the unchanged reservation; with committed intent, finish/cancel the fenced D1 action and append terminal status; with ledger outage/ambiguity, retain the fence, alert, and expose retryable maintenance status—never guess. Reservation release is compare-and-delete on user/kind/entity/op/generation. Crash-point fixtures cover after reservation, during external write, after intent, after D1 apply, and after completion append; no lease can remain silently permanent.
