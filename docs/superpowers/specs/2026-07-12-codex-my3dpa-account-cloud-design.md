...
The client generates a P-256 signing key in Keychain/Secure Enclave, Android Keystore, or non-exportable Web Crypto storage; D1 stores only the public key. Signed account calls include device ID, strictly increasing request counter, timestamp, canonical body hash, and ECDSA signature over method/path/body hash/device ID/counter/timestamp. In the same D1 transaction as the request, the Worker verifies the public key and advances `last_request_counter`. Lower counters or same-counter/different-hash requests are replay errors. An exact same-counter retry is handled by method class: mutating endpoints return their cached compact response; read-only `GET/HEAD` endpoints re-execute with the same validated parameters and never cache a response body. Pull remains safe because revisions are append-ordered and the same `after/limit` returns the same prefix plus at most later appended rows. Registering or rotating a key requires a Firebase token with recent `auth_time`, so a revoked installation cannot simply invent a new device ID using a background refresh token.

Cached mutation results are bounded, content-free status metadata for normal push, single-op lifecycle submit, device register/rotate/revoke, export start/verify, and account delete start—never entity payload/notes/state, export bytes, or pull/account/device bodies. Allowed fields are codes, IDs, versions/revisions, hashes, field masks, request IDs, retry timing, and links to authenticated live reads. `last_mutation_result` is canonical UTF-8 JSON capped at exactly 65,536 bytes, and every mutating request carries a UUID `requestId`. An exact same-counter lifecycle retry returns cached `{requestId, status:'pending_lifecycle'|terminal}`; the status endpoint remains authoritative after the cache is overwritten. Per-op metadata is bounded so the compact response fits; any larger content-free detail stays in normalized rows and is returned by the authenticated read-only status resource keyed by request ID, never truncated silently.
...
2. delete every user-scoped row—including active/tombstoned entities, lifecycle reservations, projections, `deletion_graveyard_buckets`, `sync_ops`, `system_lifecycle_ops`, devices, export jobs/items/artifacts, and entitlements—then record `domain_deleted`. Purchase rows/handles are deleted too unless a documented Art. 6(1)(c) statutory or active refund/chargeback window requires a minimal retention record; in that case atomically move only the required provider/financial fields and encrypted reconciliation handle to the UID-free `purchase_retention` store described in §12.3, then delete the account-linked row. Explicitly **exclude and retain** the `users` row with `status='deleting'` and the deletion job so the API lock survives through identity deletion. Schema migration tests fail if a new user-scoped table is absent from this enumerated cascade/check;
...
4. append completion to the already-durable external erasure record; only then delete the retained `users` row, compact the D1 job to a content-free deletion receipt (`requestId`, status, timestamps, error class only), irreversibly discard the UID from D1, and retain the receipt for 30 days before deletion.
...
Time Travel and encrypted recovery exports are inaccessible to the application and are used only for disaster recovery, never to recover an individual deleted account. A separate append-only erasure ledger lives outside the D1/backup restore boundary (private R2 objects or an equivalently isolated store). It records `requestId`, `HMAC(erasureKey, firebaseUid)`, `keyVersion`, pending/completed status, timestamps, and expiry—no raw UID or content—and is retained for the longest recovery window plus 15 days. Dedicated Worker keys and old versions remain available to the restore reconciler through the maximum ledger/recovery retention, with rotation/destruction recorded in the runbook.
...
Readers must round-trip unknown envelope and payload fields byte-for-value through their raw representation. Mutable sync writes are field-mask updates, not blind replacement: the operation carries `changedFields`, the complete values for those fields, and the entity `baseVersion`; the Worker merges only those known paths into the current payload. The rule “a client may not name an unsupported field” applies to `changedFields` writes, not opaque full-create round trips. A new/imported `version:0` create must preserve unknown payload fields verbatim and declare their original `schemaVersion`; the Worker accepts it only when the server supports that schema version, known fields validate, total/per-value bounds pass, and the canonical payload hash matches the staged/imported bytes. Otherwise return `426 schema_write_unsupported` without dropping fields. Full replacement of an existing entity is allowed only when the client supports the stored server entity schema version. This prevents an older client from erasing fields added by a newer one while preserving portable future data.
...
- `export_snapshot` is the sole non-UUID entity. `snapshotSchemaVersion` is a JSON integer from 1 through 65,535; its hash encoding is unsigned base-10 ASCII with no sign, decimal point, or leading zeros. At creation, freeze that version and the complete RFC-8785 canonical payload bytes (including then-known/additive fields) verbatim; `hashInput` is exact bytes `ASCII("3dpa-export-snapshot") || 0x00 || ASCII(canonicalDecimalVersion) || 0x00 || canonicalPayloadBytes`, then `id = "sha256:" + lowercaseHex(SHA-256(hashInput))` using the complete digest. Store those original bytes unchanged. A future schema reader may preserve/interpret them but never reserialize an old snapshot to recompute its ID; a migrated payload creates a new snapshot/version/ID. `restore_content_addressed` submits the frozen version+bytes, and fixtures publish exact prefix/payload bytes and expected ID for every supported snapshot schema.
...
`deletedAt` is authoritative server UTC recorded in the same D1 transaction as `deletedRevision`. A tombstone becomes eligible only when `deletedAt <= serverNow - 30×24 hours`; a late compaction job leaves it visible longer, never shorter. The compaction transaction writes/verifies graveyard membership before removing the full tombstone and only then advances `min_available_revision`. Boundary-clock, delayed-job, failure-between-steps, and cursor-expiry fixtures use an injected server clock.
...
Lookup is fail-closed on key availability. Before any create, restore, portable-import remap, or bootstrap resurrection check, load every `keyVersion` referenced by that account's buckets. If any version is unavailable/unverifiable, return retryable `503 graveyard_key_unavailable`, apply no entity/outbox mutation, alert, and pause compaction/key retirement; never interpret an uncomputable locator as “not present.” Pull, export, and account deletion remain available so the user is not trapped. Recovery restores the exact key version or deletes the account—there is no bypass flag.
...
2. conditionally create the per-op external decision object as `committed_intent` (`If-None-Match: *`) containing op ID, keyed user/entity locators, kind, action, base version, lifecycle generation, key version, and time. The only competing terminal decision is `cancelled_expired`; exactly one may win. On explicit conditional loss, strongly read the winner: continue if committed, or release only after proving cancelled. A timeout, 5xx, disconnect, or unreadable response is an **unknown outcome**—retain/renew the matching fence and schedule reconciliation; never cancel from transport failure;
3. retryably revalidate the action's current-base and reference preconditions, then apply in D1 by atomically allocating the then-current next `user_revision`, incrementing `entity_version`, changing the entity, and removing the reservation; append external `completed` with `appliedRevision` and `appliedEntityVersion`. If a precondition legitimately changed, remove the reservation without mutation and append `cancelled_precondition`, which DR never applies. No sync revision is reserved early, so unrelated writes/pulls cannot cross a revision gap. Because the intent is written only after an entity fence, DR may safely finish a committed intent even if live D1 completion was interrupted.

The restore-independent R2 lifecycle ledger is retained for the longest D1/recovery-export window plus 15 days. `delete` and schema-defined neutral reset are content-free. A `reactivate` event also retains the immutable export-snapshot payload encrypted with a dedicated recovery key for that bounded window, so DR can reconstruct rather than merely avoid re-deletion. DR groups by keyed `(user, kind, entity)` locator and orders lifecycle events by generation; `cancelled_precondition` terminally supersedes its intent and is never applied. A completed action is replayed only when restored state is at or before its `appliedRevision/appliedEntityVersion`; an entity with a provably later ordinary server revision/version causally supersedes that lifecycle action and is preserved. Equal versions must match the recorded delete/reset/reactivate outcome or promotion blocks. For an unresolved `committed_intent` without applied fields, DR applies it only when the restored entity exactly matches `baseVersion`; if it already exactly matches the intended next state, recovery records completion, and any other state blocks promotion. Before promotion DR assigns fresh contiguous sync revisions only to actions it actually reapplies. Fixtures cover failed external write, pending→D1 conflict prevention, unrelated write+pull during lifecycle I/O, reset→ordinary update→restore before/reset/after update, missing completion acknowledgement, committed delete before/after restore point, and delete→reactivate across DR.

A live reconciler scans expired lifecycle leases at least once per minute and also schedules on repeated `lifecycle_in_progress` or any unknown step-2 outcome. For the exact `op_id/lifecycle_generation`, it uses a strongly consistent conditional-create decision object: if none exists, it first attempts to create terminal `cancelled_expired` with `If-None-Match: *` and releases the unchanged reservation only after that wins; if `committed_intent` won, it finishes/cancels the fenced D1 action and appends terminal status. A delayed writer cannot replace `cancelled_expired`; a reconciler that loses to committed intent cannot release the fence. With ledger outage/ambiguous conditional semantics, retain/renew the fence, alert, and expose retryable maintenance status—never guess. Reservation release is compare-and-delete on user/kind/entity/op/generation. Gate C0 verifies the chosen R2/coordination primitive provides the required conditional create; otherwise use a per-op Durable Object coordinator. Crash fixtures include write-persisted-but-response-timeout, delayed external completion after lease expiry/cancellation, every decision race, after D1 apply, and after completion append; no lease can remain silently permanent or become unfenced with a committed intent.
...
users(user_id PK, status, pdm_version, next_revision, min_available_revision,
      next_lifecycle_generation, next_system_gc_sequence, app_account_token,
      created_at, deleted_at)
devices(user_id, device_id, signing_public_key, last_request_counter,
        last_request_hash, last_mutation_result, last_processed_op_sequence,
...
system_lifecycle_ops(user_id, actor, request_id, op_id, op_sequence, kind,
                     entity_id, result_status, result_json, created_at,
                     PK(user_id, actor, op_sequence), UNIQUE(user_id, op_id))
...
purchase_retention(retention_id PK, provider, environment, provider_event_id,
                   handle_encrypted, key_version, amount_minor, currency,
                   transaction_at, validation_status, retention_reason,
                   erase_after, UNIQUE(provider, environment, provider_event_id))
...
The external account-erasure and entity-lifecycle ledgers are deliberately absent from this D1 schema. Their append-only schemas are `(requestId, uidLocator, keyVersion, status, pendingAt, completedAt?, expiresAt)` and `(opId, uidLocator, kind, entityLocator, action, baseVersion, lifecycleGeneration, appliedRevision?, appliedEntityVersion?, keyVersion, status, intentAt, completedAt?, encryptedRecoveryPayload?, expiresAt)`; keyed-locator/recovery key versions remain available for the full retained-ledger window.
...
GET  /api/v1/sync/entity/{kind}/{entityId}  (authenticated live conflict read)
...
Every `sync_ops.request_id` equals the UUID in the signed push/lifecycle request; one normal batch maps that ID to several rows. `result_status` plus bounded canonical `result_json` persist the exact typed **content-free** applied/duplicate/conflict/rejected/error metadata atomically with each op and remain until the device acknowledgement/compaction rule in §14.3. `GET /sync/push/{requestId}` requires the same verified user and registered device, then reconstructs `{requestId, status, results[]}` ordered by `op_sequence`. Normal `/sync/push` results are terminal-only; the single lifecycle row may be `pending_lifecycle` until its saga reaches a terminal status. Conflict metadata includes current version/revision/hash and an entity-read path, never payload. The authenticated entity endpoint returns current live content only from `sync_entities`; after deletion it returns the content-free tombstone/not-found. Unknown/foreign IDs return not found without leaking ownership. Fixtures lose the response, restart the app, overwrite `last_mutation_result` with another request, and still recover/ack exact metadata; a conflict containing notes followed by live/DR deletion proves no result cache or backup retains those notes.
...
Each artifact uses a fresh CSPRNG 256-bit data-encryption key and AES-256-GCM with a fresh random 96-bit nonce; the binary envelope is `version|kekVersion|nonce|ciphertext|16-byte-tag`, and AAD is `3dpa-export|requestId|exportRevision|payloadHash`. Never reuse a key/nonce pair. `artifact_key_encrypted` wraps that data key using AES-256-GCM under a dedicated versioned 256-bit export KEK Worker secret, its own random 96-bit nonce, and AAD `3dpa-export-key|requestId`; it stores `{v,kekVersion,nonce,ciphertext,tag}`. KEK rotation creates a new version for new jobs, retains old versions until every artifact/job using them has expired plus 30 days, and is rehearsed before retiring a key. Decrypt authentication failure is terminal/quarantined, never plaintext fallback; R2 remains private and server-side encryption is defense in depth.

On completion, set `export_jobs.expires_at = completedAt + 7 days` and return/display that exact deadline in every status response and deletion preflight. Delete `export_job_items` staging rows within 24 hours of successful R2 materialization; retain the private R2 artifact only until `expires_at`, then delete it and mark the job `expired`. Job metadata/receipt may remain content-free for 30 days for support/audit. Expiry never traps deletion: the user can retry export or explicitly continue without it. Jobs are idempotent by request ID and expose typed terminal errors. Deletion preflight handles pending, failed, expired, completed-but-not-downloaded, verified, and explicit-cancel states before the account lock; after lock, no account export artifact remains reachable. Fixtures mutate live data during materialization/queue retries and prove the archive equals exactly `exportRevision`. R2/Queue resources are therefore activated at the account-lifecycle gate, not for normal sync payload storage.
...
`delete`, `neutral_reset`, and `restore_content_addressed` are not normal batch ops. `/sync/push` rejects a batch containing any of them with `400 lifecycle_requires_endpoint` before fence preflight/mutation. `/sync/lifecycle` accepts exactly one signed next-contiguous device op and `requestId`; its first D1 transaction writes `sync_ops.result_status='pending_lifecycle'` plus the lifecycle reservation but does **not** advance `last_processed_op_sequence`. The saga in §8.4 then updates that same row to one terminal result and advances the device watermark atomically. Retries/lost responses return the same pending/terminal status via `GET /sync/push/{requestId}`. While sequence N is pending, any N+1 operation/dependency returns retryable `409 prior_lifecycle_pending` and cannot mutate; acknowledgement is allowed only after terminal resolution.

Internal snapshot GC is not a fake client device. A trusted Worker-only path atomically allocates `users.next_system_gc_sequence`, writes `system_lifecycle_ops` with fixed actor `snapshot_gc`, `opId = system:snapshot_gc:<sequence>`, and runs the same reservation/external-decision/apply state machine. It has no `devices` row, signing key, or client-visible registration/bypass. Public request schemas reject every `system:` device/op ID; only the internal module can create system rows. Its terminal result is retained through the DR window and reconciled by request/op ID; fixtures cover sequence uniqueness, retry, crash, and proof that external callers cannot impersonate the actor.
...
- Deleting an `export_preset` **or** changing its `exportSnapshotId` records the prior snapshot as a GC candidate. After the preset mutation commits, a transaction rechecks all active preset references; if zero, it tombstones the old snapshot through the fenced lifecycle protocol. Concurrent repoint/delete/create cannot tombstone a newly referenced snapshot. Fixtures cover repoint A→B, last/non-last reference, retry, and a concurrent new reference.
- `restore_content_addressed` is the sole graveyard exception and follows the exact hash/ID verification plus fenced lifecycle protocol in §8.4; any mismatched payload returns `409 immutable_id_conflict`. Latest-per-key reset uses the fenced versioned neutral-value action, never delete/recreate.
...
- Mutable updates merge only the validated field mask. A full replacement from a client below the stored entity schema version returns `426 schema_write_unsupported` instead of dropping unknown fields.
- Stale mutable base: return content-free `409 conflict` metadata (current version/revision/hash + live entity-read path); do not overwrite or cache the current server payload.
...
Percent remaining is a presentation derived from `balanceMg / capacityMg`. CSV/import adapters normalize source units to integer milligrams and report rounding. For CSV, compute `importManifestHash = SHA-256(raw file bytes)` and derive spool/acquire-event IDs in namespace `pdm2Import` from `csv:spool:<hash>:<parsedRowIndex>` and `csv:acquire:<hash>:<parsedRowIndex>`. Re-importing identical bytes therefore deduplicates both; changed files show a dry-run duplicate/near-match preview rather than guessing identity. Fixtures cover exact re-import, reordered/edited rows, blank/header rows, and partial retry.
...
Refund, revocation, expired test entitlement, or an import may leave a free account above 50 active spools. The system never deletes, hides, or stops backing up those records: reading, export, retirement, consumption/adjustment of existing spools, and sync of existing records remain available. While over limit, creating/reactivating a spool and bulk imports that increase the active count are blocked with the exact count and two remedies: retire spools or restore Pro. Metadata edits are allowed. A platform without current store connectivity honors the last server-verified entitlement for seven consecutive 24-hour periods, measured from the server verification timestamp; app launch/local receipt checks do not reset it. A successful server verification resets the window. After expiry, apply this same non-destructive over-limit state and explain offline verification. The seven-day policy is disclosed in purchase/help copy and must be reconfirmed at the entitlement owner gate.
...
- `purchase_events` stores provider transaction/token hashes for uniqueness plus the provider reconciliation handle encrypted at rest with a dedicated, rotation-capable server key and access limited to the purchase reconciler. Decrypted Apple original transaction IDs / Google purchase tokens are never returned to clients, logs, analytics, or account exports. Retain account-linked handles only while the entitlement is active plus the documented refund/chargeback reconciliation window, then erase or revalidate the retention basis. Immutable provider event IDs, environment, validation status, and entitlement effect remain content-minimized. Replays are idempotent; sandbox never grants production entitlement; account transfer and family/ownership behavior require explicit fixtures.
- On account deletion, no entitlement or Firebase UID survives. If and only if the recorded legal-retention schedule still applies, move the minimum provider event/transaction handle, amount/currency/time, validation status, retention reason, and `eraseAfter` into a separate `purchase_retention` record with a fresh random retention ID and no account/user locator. It is usable only for refund/chargeback/tax reconciliation, never entitlement restoration or analytics; a sweeper erases the encrypted handle at `eraseAfter` and retains any longer statutory financial record only in its further-minimized legally required form. The later entitlement owner gate records the jurisdictional window and tests deletion inside/outside it.
...
The app-managed original v1 store/transition backup becomes cleanup-eligible after a verified PDM2 export plus either (a) two successful sync cycles separated by an app/browser restart for a synced account, or (b) two successful local reopen → decode → invariant-check → round-trip validations separated by restart for a signed-out user. Show its size, validation path, and deletion date in Sync & Backup; offer **Delete transition backup now** and **Save a copy**. Automatically erase it 30 days after eligibility (or sooner on explicit local-data deletion) and record only the content-free receipt. The compatibility window in §15.4 keeps the v1 *format reader/writer*, not duplicate personal data. Failed/quarantined migrations pause the deadline and surface remediation.
...
Outboxes are scoped to `(accountScope, deviceId)` and possibly-sent bytes are immutable. Before bootstrap/import remaps anything, resend every unacknowledged sequence byte-identically until the server returns its durable terminal result, acknowledge it, then pull through the resulting revision. An op at/below `last_processed_op_sequence` is never rewritten. Server-confirmed conflicts/deletions resolve by emitting fresh post-watermark operations with new IDs; dependent rejected ops likewise resolve into one newly validated graph. If the old account/device cannot be reauthenticated, quarantine and export its outbox—never attach it to another account.

ID remapping of proven local/staged content is a graph transaction, not a single-record edit. Build the complete old→new map first; atomically rewrite entity IDs, every schema-declared reference in staged entities, and fresh (never-sent) operations; then revalidate schemas, hashes, dependency graph, account/device scope, and operation sequence before commit. A reference that cannot be rewritten quarantines the entire connected component with an exportable diagnostic—never silently drops it or uploads a stale ID. Required fixtures include lost ACK before remap, account switch with pending ops, a remapped spool plus multiple inventory events, and an export preset/snapshot chain.
