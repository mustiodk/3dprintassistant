...
Before committing an individual entity deletion, write an idempotent content-free `pending_entity_erasure` record to the restore-independent R2 ledger: request/op ID, keyed user locator, kind, keyed entity-ID locator, key version, and deletion time. If that write fails, return retryable failure without tombstoning; after D1 commit, append completed status. Retain it for the longest D1/recovery-export window plus 15 days. A DR restore scans restored entity IDs through the keyed locators and reapplies every matching deletion/tombstone before promotion. This ledger contains no payload and uses the same key-retention/rotation discipline as account erasure.

Deterministic-ID kinds have explicit lifecycle exceptions. `preference` and `tuning_dismissal` are never tombstoned: reset is a normal current-base update to the schema's neutral/default value (`value:null` or `dismissedAt:null`) at the same logical-key ID. A tombstoned/compacted `export_snapshot` may be reactivated only by `restore_content_addressed` when RFC-8785/SHA-256 recomputes exactly to its entity ID and supplied bytes; the graveyard marker remains as history, but this verified active entity is allowed and receives a new server revision. No mutable/latest-per-key entity can use that exception. Fixtures cover reset and snapshot delete → compact → identical recreate/mismatched rejection.
...
The external account/entity erasure ledgers are deliberately absent from this D1 schema. Their append-only schemas are `(requestId, uidLocator, keyVersion, status, pendingAt, completedAt?, expiresAt)` and `(opId, uidLocator, kind, entityLocator, keyVersion, status, deletedAt, completedAt?, expiresAt)`; keyed-locator key versions remain available for the full retained-ledger window.
...
- `restore_content_addressed` is the sole graveyard exception and follows the exact hash/ID verification in §8.4; any mismatched payload returns `409 immutable_id_conflict`. Latest-per-key reset uses an ordinary versioned neutral-value update, never delete/recreate.
...
- Disaster-recovery rehearsal restores D1 without the live database, reads both external account and entity erasure ledgers, re-hashes every restored user/entity locator, proves all matches are re-erased/tombstoned, and blocks promotion on any unmatched/incomplete record.
