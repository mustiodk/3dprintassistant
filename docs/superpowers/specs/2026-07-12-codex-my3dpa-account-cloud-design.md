...
2. delete every user-scoped row—including active/tombstoned entities, lifecycle reservations, projections, `deletion_graveyard_buckets`, `sync_ops`, devices, export jobs/items/artifacts, entitlements, and encrypted purchase/source transaction handles—then record `domain_deleted`; explicitly **exclude and retain** the `users` row with `status='deleting'` and the deletion job so the API lock survives through identity deletion. Schema migration tests fail if a new user-scoped table is absent from this enumerated cascade/check;
...
- `export_snapshot` is the sole non-UUID entity. At creation, freeze `snapshotSchemaVersion` and the complete RFC-8785 canonical payload bytes (including then-known/additive fields) verbatim; compute `hashInput = UTF8("3dpa-export-snapshot\u0000" + snapshotSchemaVersion + "\u0000") || canonicalPayloadBytes`, then `id = "sha256:" + lowercaseHex(SHA-256(hashInput))` using the complete digest. Store those original bytes unchanged. A future schema reader may preserve/interpret them but never reserialize an old snapshot to recompute its ID; a migrated payload creates a new snapshot/version/ID. `restore_content_addressed` submits the frozen version+bytes, and fixtures publish exact input bytes and expected ID for every supported snapshot schema.
...
A live reconciler scans expired lifecycle leases at least once per minute and also schedules on repeated `lifecycle_in_progress`. For the exact `op_id/lifecycle_generation`, it uses a strongly consistent conditional-create decision object: if none exists, it first attempts to create terminal `cancelled_expired` with `If-None-Match: *` and releases the unchanged reservation only after that wins; if `committed_intent` won, it finishes/cancels the fenced D1 action and appends terminal status. A delayed writer cannot replace `cancelled_expired`; a reconciler that loses to committed intent cannot release the fence. With ledger outage/ambiguous conditional semantics, retain the fence, alert, and expose retryable maintenance status—never guess. Reservation release is compare-and-delete on user/kind/entity/op/generation. Gate C0 verifies the chosen R2/coordination primitive provides the required conditional create; otherwise use a per-op Durable Object coordinator. Crash fixtures include delayed external completion after lease expiry/cancellation, every decision race, after D1 apply, and after completion append; no lease can remain silently permanent or become unfenced with a committed intent.
...
Before processing any batch op, the same D1 transaction checks that both its target **and every schema-declared referenced entity** have no incompatible active `entity_lifecycle_reservations` row. If a target/reference is fenced, the whole push returns retryable HTTP `409 lifecycle_in_progress` before mutation and before advancing any `last_processed_op_sequence`; the client retries identical ops under a new signed request counter after backoff. Only the internal lifecycle apply carrying the matching reservation `op_id` may consume that fence, and it revalidates action-specific preconditions (including zero active preset references before snapshot GC) in its apply transaction. Tests run target and reference creation/repoint concurrently with each lifecycle action, especially preset→snapshot during GC, and prove one side retries/cancels without a dangling reference.
