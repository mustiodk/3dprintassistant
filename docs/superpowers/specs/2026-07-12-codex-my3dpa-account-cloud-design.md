...
2. delete every user-scoped row—including active/tombstoned entities, lifecycle reservations, projections, `deletion_graveyard_buckets`, `sync_ops`, devices, export jobs/items/artifacts, and entitlements—then record `domain_deleted`. Purchase rows/handles are deleted too unless a documented Art. 6(1)(c) statutory or active refund/chargeback window requires a minimal retention record; in that case atomically move only the required provider/financial fields and encrypted reconciliation handle to the UID-free `purchase_retention` store described in §12.3, then delete the account-linked row. Explicitly **exclude and retain** the `users` row with `status='deleting'` and the deletion job so the API lock survives through identity deletion. Schema migration tests fail if a new user-scoped table is absent from this enumerated cascade/check;
