...
Cached mutation results are bounded status/IDs/per-op outcomes for push, device register/rotate/revoke, export start/verify, and account delete start—never export bytes or pull/account/device bodies. The API contract sets a maximum cached result size and routes larger deterministic detail through a status resource keyed by request ID.
