...
Cached mutation results are bounded, content-free status metadata for push, device register/rotate/revoke, export start/verify, and account delete start—never entity payload/notes/state, export bytes, or pull/account/device bodies. Allowed fields are codes, IDs, versions/revisions, hashes, field masks, request IDs, retry timing, and links to authenticated live reads. `last_mutation_result` is canonical UTF-8 JSON capped at exactly 65,536 bytes, and every mutating request carries a UUID `requestId`. Per-op metadata is bounded so the compact response fits; any larger content-free detail stays in normalized rows and is returned by the authenticated read-only status resource keyed by request ID, never truncated silently.
