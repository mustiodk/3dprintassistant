...
Cached mutation results are bounded status/IDs/per-op outcomes for push, device register/rotate/revoke, export start/verify, and account delete start—never export bytes or pull/account/device bodies. `last_mutation_result` is canonical UTF-8 JSON capped at exactly 65,536 bytes, and every mutating request carries a UUID `requestId`. Per-op message/details are bounded so the compact response fits; any larger deterministic detail stays in normalized rows and is returned by the authenticated read-only status resource keyed by request ID, never truncated silently.
