...
Before processing any batch op, the same D1 transaction checks that its `(user, kind, entityId)` has no active `entity_lifecycle_reservations` row. If any target is fenced, the whole push returns retryable HTTP `409 lifecycle_in_progress` before mutation and before advancing any `last_processed_op_sequence`; the client retries identical ops under a new signed request counter after backoff. Only the internal lifecycle apply carrying the matching reservation `op_id` may consume that fence. Tests run push concurrently with each lifecycle action and prove no staging/non-D1 adapter omits the check.
