...
// [IMPL-044 W3] Mine-tier states. `personal` is injected via
// Engine.setPersonalTuning before capture (and cleared after). ~mine-noinj
// pins byte-parity with ~safe; the others show every offset limb flowing
// through the full output surface (profile, temps, advanced, export, text).
const MINE = [
  { key: 'x1c-std04-pla~mine-noinj',
    s: { printer: 'x1c', nozzle: 'std_0.4', material: 'pla_basic' },
    personal: null },
  { key: 'x1c-std04-pla~mine',
    s: { printer: 'x1c', nozzle: 'std_0.4', material: 'pla_basic' },
    personal: { pairKey: 'x1c|pla_basic', offsets: {
  ...
  { key: 'mk4-std04-abs-cold~mine',
    s: { printer: 'mk4', nozzle: 'std_0.4', material: 'abs', environment: 'cold' },
    personal: { pairKey: 'mk4|abs', offsets: {
      nozzle_temp_delta: { value: 15,  unit: '°C', date: '2026-07-06' },
...
for (const c of MINE) {
  MATRIX.push({ key: c.key, state: stateDefault({ ...c.s, profileMode: 'mine' }), personal: c.personal });
}
...
async function main() {
  ...
  for (const { key, state, personal } of MATRIX) {
    ...
    // [IMPL-044 W3] Inject (or clear) the personal layer per state, mirroring
    // the app's per-render setPersonalTuning call. Mine states capture temps
    // with the full arg list (incl. profileMode) — the app's mine-mode call
    // shape; non-mine states keep the historical 3-arg capture byte-stable.
    Engine.setPersonalTuning(personal || null);
    ...
    const entry = {
      ...
      temps:     capture(() => state.profileMode === 'mine'
                   ? Engine.getAdjustedTemps(state.material, state.environment, state.nozzle, state.speed, state.printer, state.profileMode)
                   : Engine.getAdjustedTemps(state.material, state.environment, state.nozzle)),
