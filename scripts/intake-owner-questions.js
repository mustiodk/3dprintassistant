...
// Accept the answer block fenced OR bare. Requiring the fence produced the
// 2026-08-05 failure: the owner replied with byte-identical content minus the
// ``` markers — which is exactly what GitHub's copy button on a RENDERED code
// block hands you — and the run silently consumed nothing. Following the
// instruction ("reply with the block below filled in") guaranteed the failure.
//
// Anchoring on a top-level `answers:` line keeps parsing just as deterministic:
// prose without that key still yields nothing, and a fenced block is simply the
// same text with the fence stripped first.
function extractAnswerBlock(body) {
  const text = String(body || '');
  const fence = text.match(/```ya?ml\s*\n([\s\S]*?)```/i);
  const candidate = fence ? fence[1] : text;
  const lines = candidate.split('\n');
  const start = lines.findIndex((line) => /^\s*answers:\s*$/.test(line));
  if (start === -1) return null;
  // Stop at the first line that is neither blank nor part of the block's
  // indentation, so trailing prose in the same comment cannot leak in.
  const collected = [];
  for (let i = start; i < lines.length; i += 1) {
    const line = lines[i];
    if (i > start && line.trim() !== '' && !/^\s{2,}/.test(line)) break;
    collected.push(line);
  }
  return collected;
}
...
function parseAnswers(body) {
  const lines = extractAnswerBlock(body);
  // No `answers:` key at all is not an error — most comments are just prose.
  if (!lines) return { answers: [], errors: [] };
  const answers = [];
  const errors = [];
  let current = null;
