...
function planSync(parks, openIssues) {
  const toOpen = [];
  const existing = [];
  for (const park of parks) {
    const issue = findIssue(park.candidateId, openIssues);
    if (issue) existing.push(issue);
...
if (require.main === module) {
  try {
    ...
    if (command === 'sync') {
      ...
      console.log(`DECISIONISSUE ok=true action=sync opened=${opened} closed=${closed} existing=${result.existing.length} changed=${result.changed}`);
